...
- Choose where the button appears in **Settings → Audio Button**: on all notes, only on notes in a folder, or only on notes with a tag.
...
```
OBSIDIAN_PLUGIN_DIR=/path/to/Vault/.obsidian/plugins/audio-button
```
